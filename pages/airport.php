...
function setdeparture() {
    ...
    $lat = $_POST['startLat']; // Note: Confusing code. startLat and startLng are actually the end destination, NOT (as suggested) the start location
    $lng = $_POST['startLng'];
    ...
    if ($distance > 0 && $distance <= $maxDistance) {
        ...
        // Add new passenger from airport to airport stuff here
        // Get the data of the airport that the plane is departing from
        $departAirportdataquery = $db->query("SELECT * FROM airports WHERE lat=$planeLat AND lng=$planeLng");
        $departAirportdata = $db->fetch_row($departAirportdataquery);
        $departAirportMaxPassengers = $departAirportdata['airportpop']; // Get the current amount of passengers at the current airport
        $departAirportCityPop = $departAirportdata['citypop'];
        $departAirportID = $departAirportdata['id'];

        // Catch possible errors when retrieving departure airport
        if (!$departAirportID) {
            echo "Please note that no departure airport could be found. (this can happen if your plane is departing from your headquarters)";
            $departAirportMaxPassengers = 0;
            $departAirportCityPop = 0;
            $departAirportID = 0;
        }

        // Catch possible errors when retrieving arrival airport
        $arrivalAirportdataquery = $db->query("SELECT * FROM airports WHERE lat=$lat AND lng=$lng");
        $arrivalAirportdata = $db->fetch_row($arrivalAirportdataquery);
        $arrivalAirportID = $arrivalAirportdata['id'];

        if (!$arrivalAirportID) {
            echo "Please note that an error may have occurred when retrieving the arrival airport.";
            $arrivalAirportID = 0;
        }

        // $passengers = random (50%-100%) * min((max(reputation, 1) / 50), 1) * min((airportpop / citypop) * 10, 1) * maxpassenger;
        $random = rand(50, 100)/100;
        $passenger = $random * min(max($ir['reputation'], 1) / 50, 1) * max(min(($departAirportMaxPassengers / $departAirportCityPop) * 10, 1), 0.0000001) * $maxpassenger;

        if ($passenger > $departAirportMaxPassengers) {
            $passenger = $departAirportMaxPassengers;
        }
        ...
        $ticket = $ir['tickets1'];
        $totalWeight = $emptyWeight + ($passenger * 75); // calc the total weight (average passenger weight = 75)
       ...
       // $payout = $passenger * $ticket * max(round(floor($distance / 100) * 1.05), 1) + $freightPayout; // payout
        ...
        if ($ir['premiumdays'] >= 1) {
            ...
            $payout = $passenger * $ticket * 0.4 * max(round(floor($distance / 100) * 1.05), 1) + $freightPayout; // payout
        }
        ...
        if (count($currentCostPointsArray) >= 100) {
            $currentCostPoints = $currentCostPointsArray[count($currentCostPointsArray) - 100];
            for ($i = count($currentCostPointsArray) - 99; $i < count($currentCostPointsArray) - 1; $i++) {
        ...
        if ($airplaneData['planeUname'] == 'Not Set') {
            $flightname = $airplaneData['planeMAKE'];
        } else {
            $flightname = $airplaneData['planeUname'];
        }
        $flightendt = $activeRow['flightEndTime']+3600;
        $flightend = date("h:i A",$flightendt);
        

        // 2 percent chance of a plane problem.
        $rands = rand(1, 50);
        if ($rands == '10') {
            $serviceOutputs = [
                'Altimiter has stopped functioning.',
                'Rudder Controls are Sticking.',
                'Engine Overheating',
                'Electrical Failure',
            ...
            $randIndex = array_rand($serviceOutputs);
            $serviceOutput = $serviceOutputs[$randIndex];
        
            $db->query("UPDATE userairplanes SET planeHEALTH=20 WHERE id = $plane");
        
            $servicelog1 = time();
            $servicelog2 = $serviceOutput;
            $db->query("UPDATE userairplanes SET servicelog = CONCAT(servicelog, '<br>', '$servicelog1: $servicelog2') WHERE id = $plane");
        }
        ...
        //Script to add line with plane animation for the flight
        ?><script>
            myAnimationIcon = parent.L.icon({
                iconUrl: '<?php echo $airplaneData['planeIMAGE']; ?>',
                iconRetinaUrl: '<?php echo $airplaneData['planeIMAGE']; ?>',
                iconSize: [50, 50],
                iconAnchor: [20, 20],
            });
            
            speed = <?php echo $speed; ?> / 3.6;
            endtime = <?php echo $flight_start_time + $flight_time; ?>;
            startLat = <?php echo $planeLat; ?>;
            startLng = <?php echo $planeLng; ?>;
            endLat = <?php echo $lat; ?>;
            endLng = <?php echo $lng; ?>;
            
            line = parent.L.polyline([[startLat, startLng], [endLat, endLng]], {color: '<?php echo $ir['airlinecolour']; ?>'}).addTo(parent.map);
            line.setText('           \u27A4 Flight: <?php echo number_format($plane); ?> - <?php echo $flightname; ?> (ETA: <?php echo $flightend; ?>) \u27A4', {
                repeat: true,
                offset: 12,
                attributes: {fill: 'black'}});
            

            parent.aircraftMarkers.push([parent.L.marker([startLat, startLng], {icon: myAnimationIcon,}).addTo(parent.map), vectorFromLatLong(startLat, startLng, endLat, endLng), speed, line, endtime, <?php echo $plane; ?>]);

            for (let i = 0; i < <?php echo (time() - $flight_start_time); ?>; i++) {
                updateAirplaneAnimationNoCaller(aircraftMarkers.length - 1);
                
                if (endtime <= ((new Date().getTime())/1000)) {
                    parent.map.removeLayer(aircraftMarkers[aircraftMarkers.length - 1][0]);
                    parent.map.removeLayer(aircraftMarkers[aircraftMarkers.length - 1][3]);
                    aircraftMarkers.splice(aircraftMarkers.length - 1, 1);
                    break;
                }
            }
        </script><?php
    ...
    $flighttime = date("H:i:s", $flight_time);
    ...
    if ($distance > 0 && $distance <= $maxDistance) {
        ...
        $db->query("UPDATE userairplanes SET planeACTIVE=1, planeLOCATIONLAT=$lat, planeLOCATIONLON=$lng,planeCURRENTWEIGHT=$totalWeight,planePASSENGERCURRENT=$passengers,planeFUELCURRENT=planeFUELCURRENT-$fuel,planeDISTANCETRAVELLED=$newPlaneDistancetravelled, 
                                            planeMONEYMADE=planeMONEYMADE+$payout, planeHEALTH=$newPlaneHealth, planeMAXHEALTH=$newMaxPlaneHealth, flighttime=flighttime+'$flight_time', totalflights=totalflights+'1' WHERE id=$plane");
        $db->query("UPDATE users SET bucks=bucks+$payout, totalmoney=totalmoney+$payout, totaldistance=totaldistance+$distance, reputation=reputation+$reputation, reputationa='$currentCostPoints' WHERE userid=$userid"); // pay the cash and Rep Gain
        ...
        $db->query("UPDATE airports SET airportpop=airportpop-$passenger WHERE id=$departAirportID");
        $db->query("UPDATE airports SET airportpop=airportpop+$passenger WHERE id=$arrivalAirportID");
    }
